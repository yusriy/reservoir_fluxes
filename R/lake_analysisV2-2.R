...
data_group <- data %>% 
  ...
  summarise(LE=mean(LE,na.rm=TRUE), H = mean(H,na.rm=TRUE), Z.L = mean(Z.L,na.rm=TRUE),
            ...
            qs=mean(qs,na.rm=TRUE),qa=mean(qa,na.rm=TRUE),deltaQ=mean(deltaQ,na.rm=TRUE),P = mean(Press_CS115_Avg,na.rm=TRUE),
...
# f) Fall: Specific humidity
par(mai=c(0,0.6,0.1,0))
plot(data_group$hour[data_group$season == 1],
     data_group$qs[data_group$season == 1],
     ylab='q',xlab='',type='l',lwd=2,ylim=c(0,0.035),cex.lab=2)
lines(data_group$hour[data_group$season == 1],
      data_group$qa[data_group$season == 1],
      ylab=expression('q'['a']),type='l',lty=2,lwd=2)
lines(data_group$hour[data_group$season == 1],
      data_group$deltaQ[data_group$season == 1],
      ylab=expression(paste(Delta,'q')),type='l',lty=3,lwd=2)
text(0,0.033,'f)',cex=1)
legend(20,0.035,y.intersp=1,bty='n',lty=c(1,2,3),lwd=c(2,2,2),
       c(expression('q'['s']),expression('q'['a']),expression(paste(Delta,'q'))),cex=1)
minor.tick(ny=2,nx=5,tick.ratio=0.5)

# l) Winter: Specific humidity
par(mai=c(0,0.3,0.1,0.1))
plot(data_group$hour[data_group$season == 0],
     data_group$qs[data_group$season == 0], main='',
     ylab='',xlab='',type='l',lwd=2,ylim=c(0,0.035),yaxt='n',cex.lab=2)
lines(data_group$hour[data_group$season == 0],
      data_group$qa[data_group$season == 0],
      ylab=expression('q'['a']),type='l',lty=2,lwd=2)
lines(data_group$hour[data_group$season == 0],
      data_group$deltaQ[data_group$season == 0],
      ylab=expression(paste(Delta,'q')),type='l',lty=3,lwd=2)
text(0,0.033,'l)',cex=1)
legend(20,0.035,y.intersp=1,bty='n',lty=c(1,2,3),lwd=c(2,2,2),
       c(expression('q'['s']),expression('q'['a']),expression(paste(Delta,'q'))),cex=1)
minor.tick(ny=2,nx=5,tick.ratio=0.5)
