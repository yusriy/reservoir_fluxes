...
data<-cbind(data,qs,qa,deltaQ,deltaE,u_deltaE,C_E)
...
# J. Calculate LE/U to remove the affect of U on LE
LE_U <- data$LE/data$WS_Spd_WVT
data <- cbind(data,LE_U)
rm(LE_U)
...
##### 5D. Fig. 5: LE/U for different ASL stability ranges ####
# Path where the plots will be saved
...
plot6 <- ggplot(na.omit(data[,c('LE_U','stability_no')]), aes(factor(stability_no),LE_U)) + geom_boxplot(outlier.size=0,fill="white") + 
  geom_jitter(alpha=I(1/8),aes(color=factor(stability_no))) + 
  stat_summary(fun.y="mean",colour='black',geom='text',label='-----',size=10) + 
  labs(x="ASL stability ranges",y='LE/U') + theme_bw() +
  theme(legend.position='none',panel.grid.major=element_line(size=0),panel.grid.minor=element_line(size=0),
        axis.title.y=element_text(size=12),axis.title.x=element_text(size=12),
        axis.text.y=element_text(size=12),axis.text.x=element_text(size=12)) + 
  scale_x_discrete(labels=names_boxplot) +
...
plot6 <- ggplot(data=data_rsq,aes(x=cat_no,y=r_LE_U)) +
...
plot7 <- ggplot(data=data_rsq,aes(x=cat_no,y=r_LE_deltaE)) +
  geom_bar(stat='identity',fill='white',color='black',position='identity') +
  xlab('') + ylab('r') + ylim(-0.05,1.0) +
  theme(axis.title.y=element_text(size=14),
...
#regression R^2 results plot between LE and deltaE
plot8 <- ggplot(data=data_rsq,aes(x=cat_no,y=r_LE_udeltaE)) +
  geom_bar(stat='identity',fill='white',color='black',position='identity') +
  xlab('') + ylab('r') + ylim(-0.05,1.0) +
  theme(axis.title.y=element_text(size=14),
...
multiplot2(plot6,plot7,plot8,cols=1,labs=list('ASL stability ranges',''))
dev.off()

##### 5F. Fig. 7: LE against U, deltaE, and u_deltaE in different ranges ####
...
##### 5E. Fig. 8: In "CE and stability" paper ####
## Starting fresh
par(old.par)
# Path where the plots will be saved
...
plot6 <- ggplot(na.omit(data[,c('C_E','stability_no')]), aes(factor(stability_no),C_E)) + geom_boxplot(outlier.size=0,fill="white") + 
  geom_jitter(alpha=I(1/8),aes(color=factor(stability_no))) + 
  stat_summary(fun.y="mean",colour='black',geom='text',label='-----',size=10) + 
...
##### 5G. Fig. 9: C_E against LE, U, deltaE, and u_deltaE ####
## Starting fresh
par(old.par)
# Path where the plots will be saved
...
plot7 <- ggplot(data=data,aes(x=LE,y=C_E,group=stability,color=stability,shape=stability)) + geom_point(size=3,na.rm=TRUE) + scale_color_manual(values=c('blue','red')) +
  scale_y_continuous(breaks=seq(-0.01,0.03,by=0.01),limits=c(-0.01,0.03)) + scale_x_continuous(breaks=seq(-100,400,by=100),limits=c(-100,400)) +
  theme(panel.background=element_rect(fill='white',color='black'), panel.grid.major=element_line(size=0,color='gray'),
        panel.grid.minor=element_line(size=0,color='gray'),axis.title.x=element_text(size=14,color='black'),axis.title.y=element_text(size=12,color='black'),
        axis.text.x=element_text(size=14,color='black'),axis.text.y=element_text(size=12,color='black'),legend.position='none') + annotate('text',x=-100,y=0.03,label='a)') +
  ylab(expression(paste('C'[E]))) + xlab('LE')

# (b) Plotting C_E against U
plot8 <- ggplot(data=data,aes(x=WS_Spd_WVT,y=C_E,group=stability,color=stability,shape=stability)) + geom_point(size=3, na.rm=TRUE) + scale_color_manual(values=c('blue','red')) +
  scale_y_continuous(breaks=seq(-0.01,0.03,by=0.01),limits=c(-0.01,0.03)) + scale_x_continuous(breaks=seq(0,14,by=1)) +
  theme(panel.background = element_rect(fill='white',color='black'), panel.grid.major=element_line(size=0,color='gray'), 
        panel.grid.minor=element_line(size=0,color='gray'),axis.title.x=element_text(size=14,color='black'),axis.title.y=element_text(size=12,color='black'),
        axis.text.x=element_text(size=14,color='black'), axis.text.y=element_text(size=12,color='black'),legend.position='none') + annotate('text',x=0,y=0.03,label='b)') +
  ylab(expression(paste('C'[E]))) + xlab('U')

# (c) Plotting C_E against deltaE
plot9 <- ggplot(data=data,aes(x=deltaE,y=C_E,group=stability,color=stability,shape=stability)) + geom_point(size=3,na.rm=TRUE) + scale_color_manual(values=c('blue','red')) +
  scale_y_continuous(breaks=seq(-0.01,0.03,by=0.01),limits=c(-0.01,0.03)) + scale_x_continuous(breaks=seq(-0.5,3,by=0.5),limits=c(-0.6,3.0)) +
  theme(panel.background=element_rect(fill='white',color='black'), panel.grid.major=element_line(size=0,color='gray'),
        panel.grid.minor=element_line(size=0,color='gray'),axis.title.x=element_text(size=14,color='black'),axis.title.y=element_text(size=12,color='black'),
        axis.text.x=element_text(size=14,color='black'),axis.text.y=element_text(size=12,color='black'),legend.position='none') + annotate('text',x=-0.6,y=0.03,label='c)') +
  xlab(expression(paste(Delta,'e'))) + ylab('')

# (d) Plotting C_E against u_deltaE
plot10 <- ggplot(data=data,aes(x=u_deltaE,y=C_E,group=stability,color=stability,shape=stability)) + geom_point(size=3,na.rm=TRUE) + scale_color_manual(values=c('blue','red')) +
  scale_y_continuous(breaks=seq(-0.01,0.03,by=0.01),limits=c(-0.01,0.03)) + scale_x_continuous(breaks=seq(-5,15,by=5),limits=c(-5,15)) +
  theme(panel.background=element_rect(fill='white',color='black'), panel.grid.major=element_line(size=0,color='gray'),
        panel.grid.minor=element_line(size=0,color='gray'),axis.title.x=element_text(size=14,color='black'),axis.title.y=element_text(size=12,color='black'),
        axis.text.x=element_text(size=14,color='black'),axis.text.y=element_text(size=12,color='black'),legend.position='none') + annotate('text',x=-5,y=0.03,label='d)') +
  xlab(expression(paste('U',Delta,'e'))) + ylab('')

multiplot(plot7,plot8,plot9,plot10,cols=2)
dev.off()

#### 5H. Fig. 10: C_E against U and u* ####
...
##### 5G. Fig. 11: C_E against z/L #####
par(old.par)
# Path where the plots will be saved
path_fig <- file.path('/Users/Yusri/Documents/Work/Data analysis/lake/figs/figs_V3/fig_11.jpg')
jpeg(file=path_fig,width=450,height=1000)
## Creating a new plot
plot.new()
par(mfrow=c(3,1))

plot(data$Z.L,data$C_E,ylim=c(-0.001,0.006),xlim=c(-10,10),xlab='z/L',ylab=expression('C'[E]),pch=19)
points(data$Z.L,data$C_E_estim,col='red',pch=19)
text(-10,0.006,'a)',cex=1.5)

plot(data$C_E,data$C_E_estim,pch=19,xlab=expression(paste('C'[E],' measured')),ylab=expression(paste('C'[E],' predicted')),
     ylim=c(-0.0001,0.006),xlim=c(-0.0001,0.006))
abline(c(0,0),c(1,1),lwd=2)
text(0,0.006,'b)',cex=1.5)

plot(data$Z.L,data$diff,pch=19,xlab=expression(zeta),ylab=expression(paste('C'[E],' measured', ' - ', 'C'[E], ' predicted')),
     ylim=c(-0.10,0.10))
text(-9.2,0.10,'c)',cex=1.5)
...
rm(path_fig,plot1,plot2,plot3,plot4,plot5,plot6,plot7,
   plot8,plot9,plot10,plot11,names_boxplot,i)
rm(old.par)
