...
plot1 <- ggplot(na.omit(d[,c('LE','stability_no')]), aes(factor(stability_no),LE)) + geom_boxplot(outlier.size=0,fill="white") + 
  geom_jitter(alpha=I(1/8),aes(color=factor(stability_no))) + 
  stat_summary(fun.y="mean",colour='black',geom='text',label='---',size=7) + 
  labs(x="",y="LE") + theme_bw() + annotate("text",x=0.8,y=270,label="a)",size=7,family='Times') +
  ...
  coord_cartesian(ylim = c(-100, 310)) +
